...
handleDigitKeys msg model nonDigitHandlers =
  ...
  in
    ...
    then 
      let 
        newModel = handleDigitInput digit model
      in
...
handleSetPrecisionKeys msg model remainingHandlers =
  let 
    ( digit, handled ) = 
      case msg of 
        SetPrecision_7_Key          -> (  7, True  )
        SetPrecision_8_Key          -> (  8, True  )
        SetPrecision_9_Key          -> (  9, True  )
        SetPrecision_4_Key          -> (  4, True  )
        SetPrecision_5_Key          -> (  5, True  )
        SetPrecision_6_Key          -> (  6, True  )
        SetPrecision_1_Key          -> (  1, True  )
        SetPrecision_2_Key          -> (  2, True  )
        SetPrecision_3_Key          -> (  3, True  )
        SetPrecision_0_Key          -> (  0, True  )
        SetDisplayScientific_Key    -> ( 10, True  )
  ...
  in
    ...
    then 
      let 
        newModel = update_Display_Precision digit model 
      in
        ( { newModel | message = Basics.toString msg  }, Cmd.none )
    else
...
handleUnaryOperatorKeys msg model remainingHandlers =
  let 
    ( unary_op, handled ) = 
      case msg of 
        LN_Key                -> ( natural_log,       True  )
        Square_Root_Key       -> ( square_root,       True  )
        Reciprocal_Key        -> ( reciprocal,        True  )
        E_to_the_x_Key        -> ( e_to_the_x,        True  )
        FRAC_Key              -> ( fractional_part,   True  )
        INTG_Key              -> ( integral_part,     True  )
        X_Squared_Key         -> ( x_squared,         True  )
        N_Factorial_Key       -> ( n_factorial,       True  )
        RND_Key               -> ( round_function model.displayPrecision, True )

        _                     -> ( Basics.identity,   False )
  in
    if ( handled )
    then 
      let 
        newModel = unaryOperator unary_op model 
      in
        ( { newModel | message = Basics.toString msg  }, Cmd.none )
    else
        chainHandler msg model remainingHandlers
...
handleBinaryOperatorKeys msg model remainingHandlers =
  let 
    ( binary_op, handled ) = 
      case msg of 
        Divide_Key            -> ( y_divided_by_x, True  )
        Y_toThe_X_Key         -> ( y_to_the_x,     True  )
        Multiply_Key          -> ( y_times_x,      True  )
        Subtract_Key          -> ( y_minus_x,      True  )
        Sum_Key               -> ( y_plus_x,       True  )
  ...
  in
    ...
    then 
      let 
        newModel = binaryOperator binary_op model
      in
        ( { newModel | message = Basics.toString msg  }, Cmd.none )
    else
        chainHandler msg model remainingHandlers
...
newUpdate msg model =
  case msg of
    ...
    CLEAR_REG_Key         ->
      let 
        newModel = clearAllRegisters model 
      in
        ( { newModel | message = Basics.toString msg  }, Cmd.none )
